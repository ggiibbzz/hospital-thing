...
def actionChecker(state0, L, E, S):
    ...
    for action in S.actions:
        if L.isOverUt(state0) == True:
            ...
            actionwithactionstates.append((action, tuple(actionstates)))
    ...
    for action in S.actions:
        ...
        allpossactsdistrsnodupes = f7(tuple(distrs))
        print("allpossdistrsnodupes ", allpossactsdistrsnodupes)
    ...
    allpossibleactions = ()
    print("statewithactionstates",statewithactionstates)

    for i in range(len(statewithactionstates)):
        print("STATE WITH ACTION STATES ",statewithactionstates[i])
        allpossibleactions = allpossibleactions+ statewithactionstates[i][1]
    allpossibleactions = tuple(f7(allpossibleactions))
    return allpossibleactions, tuple(allpossactsdistrsnodupes)
...
def posStates(state0, L, E, S):
    ...
    actions = actionChecker(state0, L, E, S)
    print('posStates actions ',actions)
    if len(actions) == 1:
...
def stateSpace(state0, L, E, S):
    ...
    while len(nextstates) > len(statelist):
        ...
        for uniquestate in uniquestates:
            ...
            nextstates = f7(tuple(nextstates))
            print(len(uniquestates), i, nextstates)
        nextstates = statelist + nextstates
        nextstates = f7(tuple(nextstates))
...
def main():
    ...
    print("All transitions ",transitioner((0,0),L,E,S))
    print(posStates((0,0), L, E, S))
