...
trans_probs = np.array([[[0.8, 0.2], [0.0, 1.0]], [[0.2, 0.8], [0.0, 1.0]]])
##Entering probs = [entering in specialty 1, entering in specialty 2]
enter_probs = np.array([[0.5, 0.0]])
#######
bandwidth = 1
...
def posStates(state0, L, E, S):
    transitionstates = transitioner(state0, E, S)
    everyposstate = ()
...
def stateSpace(state0, statevector, L, E, S, c=0):
    statelist = posStates(state0, L, E, S)
    ...
    if c>3000:
        ...
        for staat in statelist:
            if staat in statevector:
                return statevector
            else:
                stateSpace(staat,statevector, L, E, S,c)
...
def main():
    ...
    print(posStates(state, L, E, S))
    print(len(statelist), statelist)
    # print(actionChecker([[3,0]], L, S)[1])
